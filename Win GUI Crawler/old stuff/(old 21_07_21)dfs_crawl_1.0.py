...
#Restricted = ["Minimise","Maximise","Minimise the Ribbon","Close","Exit","Close Calculator","Restore"] #Minimise ribbon alters rootnode structure
#Restricted=['Minimise','Help','Account', 'New', 'Hide or show region', 'Blank document', 'Options', 'Restore Down', 'Open', 'Snapshot calendar', 'Blue grey resume', 'Home', 'Pinned', 'Welcome to Word', 'Minimize', 'System', 'Shared with Me', 'Single spaced (blank)', 'Marko Savic', 'Feedback', 'Recent']
Restricted = []
union_traversed = set([])
...
class Node(object):
    ...
    def __init__(self, name, action, click_list): #action might be useless
...
if __name__ == '__main__':
    ...
    if os.path.isfile(directory+"graphs"):
        ...
        graph = {}
        nodes = {}
        rootNode = Node(rootNodeName,None,None) #Name, last action, list of unique clickables
        ...
        while DeadEnd == False:
            ...
            if current_node.visited == False:
                ...
                for clickable_el in unique_list:
                    ...
                    nodes[child_name] = Node(child_name,clickable_el,None)
